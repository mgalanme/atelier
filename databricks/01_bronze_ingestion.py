...
def write_batch(df, table_name, run_id, spark_session):
    ...
    df_with_meta = df.withColumn("ingestion_run_id", lit(run_id)).withColumn(
        "ingested_at", current_timestamp()
    )
    df_with_meta.write.format("delta").mode("overwrite").option("mergeSchema", "true").saveAsTable(
        full_name
    )
    ...
    for i in range(n):
        rows.append(
            Row(
                ...
                as_of_date=(datetime.now() - timedelta(days=random.randint(0, 30)))
                .date()
                .isoformat(),
