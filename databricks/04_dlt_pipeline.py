...
@dlt.table(name="trend_signals_bronze", comment="Raw trend signals (from existing bronze table)")
def trend_signals_bronze():
    return spark.sql("SELECT * FROM atelier.bronze.trend_signals_raw")  # noqa: F821


@dlt.table(
    name="social_listening_bronze", comment="Social listening signals (from existing bronze table)"
)
...
def trend_signals_silver():
    return dlt.read("trend_signals_bronze").select(
