"""
Registers and deploys the sustainability agent as a Mosaic AI Model Serving endpoint.
"""
...
CATALOG = "atelier"
GOLD_SCHEMA = "gold"
MODEL_NAME = f"{CATALOG}.{GOLD_SCHEMA}.sustainability_agent"
ENDPOINT_NAME = "atelier-sustainability-agent"
...
def register_and_deploy():
    input_example = pd.DataFrame(
        {
            "concept": ["A sustainable winter jacket"],
            "materials": [["recycled polyester", "organic cotton", "down"]],
            "target_markets": [["EU", "US", "UK"]],
        }
    )

    with mlflow.start_run(run_name="register-sustainability-agent"):
